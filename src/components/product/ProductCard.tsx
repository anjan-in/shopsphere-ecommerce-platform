...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <div className="group relative flex flex-col overflow-hidden rounded-xl border bg-white shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-md">
      ...
      <div className="relative aspect-square overflow-hidden bg-slate-100">
        ...
        {/* Discount Badge */}
        {hasDiscount && (
          <span className="absolute top-3 left-3 z-10 rounded-full bg-red-500 px-2.5 py-1 text-xs font-bold text-white shadow-sm">
            -{discountPercent}% OFF
          </span>
        )}
        ...
        {/* Wishlist Interactive Floating Trigger */}
        <button 
          onClick={toggleWishlist}
          className="absolute top-3 right-3 z-20 flex h-8 w-8 items-center justify-center rounded-full bg-white text-slate-600 shadow-sm transition-transform duration-200 hover:scale-110 hover:text-red-500 opacity-0 group-hover:opacity-100 focus:opacity-100"
        >
          <FaRegHeart className="h-4 w-4" />
        </button>

        {/* Quick View Hover Sheet Panel Overlay */}
        <div className="absolute inset-x-0 bottom-0 flex translate-y-full items-center justify-center bg-gradient-to-t from-black/60 to-transparent p-4 transition-transform duration-300 group-hover:translate-y-0">
          <Link 
            ...
            className="flex items-center gap-2 rounded-md bg-white px-4 py-2 text-xs font-semibold text-slate-800 shadow-xs transition hover:bg-slate-50"
          >
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        {/* Instantly Responsive Button Call To Action */}
        <button
